...
const DESTRUCTIVE_PATTERNS = [
	...
	// --- Local hardening (diverges from upstream example; see AUDIT.md) ---
	// Block network egress entirely: "read-only" must mean local-read-only.
	/\bcurl\b/i,
	/\bwget\b/i,
	/\bnc\b/i,
	/\bncat\b/i,
	...
	/\bscp\b/i,
	/\bftp\b/i,
	// Block interpreter / command-substitution escapes (awk system(), sed e, eval, xargs, etc.)
	/\bsystem\s*\(/i,
	/\beval\b/i,
	/\bexec\b/i,
	/\bxargs\b/i,
 ...
 * most specific (and most commonly surprising) cause first.
 */
export function blockReason(command: string): string | null {
	...
	if (/\bsystem\s*\(|\beval\b|\bexec\b|\bxargs\b|-exec\b|\bbash\s+-c\b|\bsh\s+-c\b/i.test(command))
		return "interpreter/exec escape not allowed";
	if (DESTRUCTIVE_PATTERNS.some((p) => p.test(command))) return "destructive command not allowed";
	return "command not on the read-only allowlist";
}
