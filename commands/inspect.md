...
- 2026-03-31 [Project A]: Edge cases pass (Pass 3) found the most actionable issues — unguarded JSON parsing, filename injection, missing input validation. Architecture pass found real concerns but lower severity (evidence: inspection Fix Now items)
- 2026-03-31 [Project A]: Parallel subagents for passes 1/3/4+5 worked well (3 agents). Spec compliance pass (Pass 2) found zero gaps — could be skipped for mature projects if inspection was run at earlier milestones (evidence: spec compliance agent found 0 gaps)
- 2026-03-31 [Project A]: Re-inspection after fixes (verify pass) caught a test needing a provider wrapper — a regression introduced by the fix. Always re-run full test suite, not just new tests (evidence: failing test after wiring change)
- 2026-05-18 [Project A]: For focused-fix sessions, the "inherited-pattern sweep" (re-grepping for other call sites of the bug class after a primitive is fixed) was the single highest-signal output of inspect — found multiple other call sites beyond the original triage scope. Consider elevating this from a paragraph buried in pass findings to a dedicated "Inherited from this change" section in the report template (evidence: inspection output)
- 2026-05-18 [Project A]: Promote-to-global step produced a rule suggestion that overlapped with existing `~/.claude/rules/minimal-diff.md`; user declined the promotion citing the overlap. The "Promote to global" prompt should pre-check `~/.claude/rules/` for keyword overlap with the proposed rule and surface conflicts in the prompt — asking the user to spot overlap manually is the failure mode (evidence: this session's discussion)
- 2026-07-07 [Project B]: Security pass called the audit baseline "drifted / ADR stale" after reading only a header — a scope extension lower in the same ADR already documented all current advisories with per-finding reachability, and even instructed /inspect to diff findings against it. The finding was substantially wrong and mislabeled the severity. Fix: when a pass flags a living doc as stale/wrong, grep the WHOLE doc for later "scope extension"/"amendment"/"update" sections before reporting — long ADRs accrete amendments below the header (evidence: this session; ADR scope extension vs header)
- 2026-07-07 [Project B]: Parallel adversarial subagents (one per shipped phase, security-focused) + the introduced-vs-inherited re-calibration worked as designed — found the one real bug, cleared the rest as sound, and correctly demoted findings for a single-user desktop app. The inherited-pattern sweep after the fix confirmed no siblings. Second project corroborating the sweep note → the "Inherited from this change" report section is worth promoting (evidence: this session; bug verified live, sweep clean)
- 2026-07-14 [Project C]: Live-behavior verification INSIDE /inspect (run the real code path, reproduce the bug with a standalone repro, quantify the consequence on live data) was the single highest-signal technique across multiple per-phase inspections — reproduced write-queue poisoning, traced CI-timeout-discards-partial-progress interaction, and quantified digest flooding on live data. Tests-pass + code-reading alone would have missed all three. 3rd project corroborating adversarial/live verification → PROMOTION CANDIDATE: add an explicit "verify live, don't just reason — reproduce and quantify" step to the pass template (evidence: project learnings docs)
- 2026-07-17 [Project C]: Highest-yield finding class this arc was VERIFICATION THEATER — protective-looking things that cannot fire (tests asserting on re-implemented logic across review rounds, CI validate step running post-checkout on the wrong tree, validate exiting 0 on duplicates). Add an explicit pass: for each safety check/test, seed the failure it guards and watch it trip. (evidence: project review rounds)
