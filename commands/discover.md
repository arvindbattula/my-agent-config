...
Ask the user for a one-line description of the idea first. Then run:

```bash
~/.claude/bin/recall "<user's one-line idea>" --budget 2500
```
...
- 2026-04-18 [innovate-intel]: "Unit of LLM extraction" (message vs thread vs document) wasn't probed. Shipped per-message in Phase 2; redesigned to thread-level post-Phase-8 for topic coherence, requiring dual-write migration across 15+ queries (evidence: docs/learnings.md "Thread-Level Extraction")
- 2026-04-18 [innovate-intel]: Discovery committed to one LLM provider (Claude). A second provider (OpenAI embeddings) surfaced in Phase 3 canonicalization, adding new env var, fallback logic, and divergent error handling. Probe for specialty capabilities (embeddings/OCR/TTS) upfront (evidence: docs/decisions.md "text-embedding-3-small")
- 2026-04-18 [innovate-intel]: Graph model cardinality (simple Graph vs MultiGraph) wasn't surfaced. Silent edge collapse in Phase 4 forced rewrite of all edge access patterns. Any "same pair, multiple relationship types" question should be asked in Layer 4 data modeling (evidence: docs/decisions.md "MultiGraph over Simple Graph")
- 2026-04-18 [innovate-intel]: Startup work blocking serving wasn't flagged. `run_processing()` in FastAPI lifespan made the server unresponsive for ~5 min with a 400-message backlog. Phase 8e/8f moved to daemon thread. Ask: "Can the server yield before long-running startup work completes?" (evidence: docs/learnings.md Phase 8e/8f)
- 2026-04-18 [innovate-intel]: Runtime-configurable constants were discovered late. 37+ hardcoded values became a settings registry in Phase 8. If more than a handful of constants need post-ship tuning, plan the registry from Phase 1 (evidence: docs/decisions.md "Settings Registry")
