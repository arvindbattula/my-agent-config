...
- 2026-04-18 [innovate-intel]: 8-phase structure held up through ship; parallel Phase 6/7 worked as planned. But Phase 8 ("Polish & Harden") absorbed far more late-discovered work than scoped — settings registry, chunking, background processing, retention. The organic split into 8a-8d then 8e/8f was correct; consider explicitly budgeting a "harden & tune" phase as 2-3 sub-phases rather than one (evidence: docs/plan.md Phase 8 + deferred-items lists)
- 2026-04-18 [innovate-intel]: "Report template design deferred to build phase" (Open Question #4) was the right call — the report format evolved three times (one-pager → 9-section enriched → 5-section decision brief) after shipping. Blueprint should explicitly tag deliverables likely to iterate post-ship, not present a single "Done when" for them (evidence: project_decision_brief_redesign memory + docs/decisions.md)
- 2026-04-18 [innovate-intel]: Plan-is-spec lesson: dashboard read-model endpoints (/briefing, /topics, /feed) were added during blueprint review after they were missed in spec. Catching this in blueprint saved a rewrite. The pattern: when the spec talks about UI views, the blueprint must explicitly design the server endpoints that back them — not assume "frontend will aggregate from primitives" (evidence: docs/learnings.md "Planning Mistakes Caught")
- 2026-07-14 [jobscout]: The 9-phase plan held up, but a whole "Pre-Phase-6 hardening" block (bounded concurrency + title pre-filter) had to be INSERTED and Phase 3 re-worked mid-build — all forced by the LLM's real per-call latency (24-43s) and first-run volume (~2,660 postings vs a 90-min CI timeout) not being modeled at plan time. When the pipeline makes an expensive EXTERNAL per-item call at scale, blueprint should explicitly budget an operational-hardening phase (concurrency, incremental persistence, rate-limits, CI-time-fit) and capture the max-run-time estimate up front. 2nd instance of "harden phase absorbs more than scoped" (with innovate-intel) (evidence: docs/plan.md Pre-Phase-6 section + docs/learnings.md Phase 3/6 inspections)
- 2026-07-17 [jobscout]: Phase acceptance criteria that demanded LIVE-DATA verification (Phase E2: 'reproduce >=3 known finds from a snapshot') caught real bugs; criteria that only said 'tests pass' let decorative tests through two review rounds. Prefer acceptance criteria stated as observable end-to-end behavior on real data. (evidence: docs/plan.md Phase E vs E5 review history)
