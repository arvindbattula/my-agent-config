...
- 2026-07-07 [agentic-rag]: "Assess current state" (Step 2) checks output-file existence but NOT git history — /construct built all of Phase 15b against session-start baseline d10a053 while the repo had already advanced to a HEAD where 15b was committed (fe11bdc). The whole phase was redundant, discovered only when an external-edit note exposed the already-present ADR-067. Fix: before building, re-read plan.md phase Status LIVE and run `git log --oneline | grep -i "phase N"` / verify HEAD hasn't moved since session start — a parallel session or a pull can land the target phase after the orientation snapshot (evidence: this session; fe11bdc already committed vs d10a053 start)
