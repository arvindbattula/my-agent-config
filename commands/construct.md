...
- 2026-03-31 [Project A]: Context window management advice was critical — later phases were deferred to a fresh session after earlier phases filled context. Fresh session produced clean code on first pass (evidence: session handoff message)
- 2026-03-31 [Project A]: Batching independent frontend fixes into parallel edits worked well. Backend fixes needed sequential handling due to shared server module (evidence: project learnings docs)
- 2026-03-31 [Project A]: Vitest fake timers + React = fragile. Two tests broke due to jsdom incompatibility. Deferred-promise pattern (control when the mock resolves) is more reliable than vi.useFakeTimers() for async polling tests (evidence: project learnings docs)
- 2026-07-07 [Project B]: "Assess current state" (Step 2) checks output-file existence but NOT git history — /construct built a phase against a stale session-start baseline while the repo had already advanced to a HEAD where that phase was committed. The whole phase was redundant, discovered only when an external-edit note exposed the already-present ADR. Fix: before building, re-read plan.md phase Status LIVE and run `git log --oneline | grep -i "phase N"` / verify HEAD hasn't moved since session start — a parallel session or a pull can land the target phase after the orientation snapshot (evidence: this session; phase already committed vs session-start baseline)
