...
- 2026-05-18 [Project A]: Initial triage missed a second code path with the same concern — Explore agent surfaced one path but stopped before finding a parallel one. Reviewer caught it in their rewrite. Lesson: triage's Explore phase should explicitly enumerate parallel pipelines (per-message + per-thread, sync + batch, single + plural) for any concern before scoping the fix — a single grep stops at the first match (evidence: reviewer rewrite called out the missed second path)
